      ...
      <div className=" flex-1 py-6 w-full bg-[#F9FAFB] rounded-x100 shadow-sm hover:shadow-md transition flex flex-col overflow-hidden">
        ...
        <div className="flex flex-col w-full md:p-6 items-center md:items-start gap-6 mt-6">
          <div className="flex flex-col md:flex-row w-full items-start gap-8">
            ...
            {/* Container de Imagens (1/3) - ESTILIZAÇÃO APLICADA */}
            {/* ALTERADO: w-full e aspect-square para mobile; md:w-96 md:h-96 para tamanho fixo em desktop */}
            <div className=" w-full md:w-1/3 flex flex-col gap-4 mt-4 md:mt-0">
              {/* Imagem Principal */}
              <div
                // ALTERADO: Classes para definir tamanho fixo (md:w-96 md:h-96)
                className="relative w-full aspect-square md:w-96 md:h-96 flex justify-center py-2 md:py-5 bg-white rounded-lg shadow-md overflow-hidden"
              >
                <Image
                  src={imageUrl}
                  width={400} // Ajuste para maior qualidade
                  height={400}
                  alt={product.nome}
                  // AQUI: w-full e h-full para preencher o quadrado, object-cover para o corte
              ...
              {product.midias && product.midias.length > 1 && (
                <div className="flex justify-center gap-2 overflow-x-auto p-2">
                  {product.midias!.map((media, index) => (
                    <div
                      key={media.id}
                      // AQUI: Classes para forçar o tamanho quadrado (w-20/h-20) e fundo cinza
                      className={`
                        cursor-pointer rounded-lg p-1 transition-all flex-shrink-0 
                        w-20 h-20 bg-gray-100 flex items-center justify-center aspect-square
                      ${index === selectedImageIndex
                          ? "border-2 border-[#3DF034] shadow-md"
                          : "border-2 border-transparent hover:border-gray-300"
                        }`}
                      onMouseOver={() => setSelectedImageIndex(index)}
                    // O style foi substituído por w-20 h-20
                    >
                      <Image
                        src={media.url}
                        width={70}
                        ...
                        // AQUI: Garante que a miniatura preencha o quadrado
            ...
            {/* Informações (2/3) */}
            {/* ALTERADO: Adicionado md:mt-4 para descer um pouco o bloco no desktop */}
            <div className="w-full md:w-1/3 flex flex-col gap-4 mt-4 **md:mt-4**">
              ...
              <p className="font-bold">Preço aproximado</p>

              <p className="text-sm text-gray-600 -mt-1">
                Verifique o valor final com o atendente
              </p>
              ...
              <div className="flex flex-col gap-4 mt-4 w-full md:w-2/3">
                ...
                <button
                  ...
                  disabled={!product || isNaN(quantity) || quantity < 1}
                  className={`
    bg-[#3cc10c] text-white text-base font-bold p-2 rounded-2xl focus:outline-none transition
