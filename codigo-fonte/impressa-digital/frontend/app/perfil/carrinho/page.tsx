...
// Tipos simplificados (mantidos dentro deste arquivo para facilitar futura refatoração)
type BackendCarrinho = { id: number; produto_id: number; user_id: number; criado_em: string };
type BackendProduto = { id: number; nome: string; descricao?: string; preco?: number };
    ...
    return (
        <div className="relative flex flex-col md:flex-row gap-4 p-4 sm:p-5 md:p-6 rounded-3xl border border-gray-300 bg-white shadow-sm">
            ...
            <div className="flex-1 flex flex-col gap-2 sm:gap-3">
                ...
                {item.customizable && (
                    <span className="inline-block bg-[#14a800] text-white text-[10px] sm:text-xs font-semibold px-2.5 sm:px-3 py-1 rounded-full tracking-wide w-fit">PERSONALIZADO</span>
                )}
                <p className="text-xl sm:text-2xl font-bold tracking-tight">{currency.format(item.price)}</p>
                <p className="text-xs sm:text-sm text-gray-600 max-w-prose leading-relaxed line-clamp-4">{item.description}</p>
    ...
    const removeItem = useCallback(async (id: string) => {
        ...
        if (/^\d+$/.test(id)) {
            try {
                ...
                // Silencia por enquanto; pode-se adicionar toast futuramente
        ...
        <div className="text-black flex flex-col gap-6 sm:gap-8 max-w-7xl mx-auto px-3 sm:px-4 py-4 sm:py-6">
            ...
            <div className="flex flex-col lg:flex-row gap-6 lg:gap-8">
                <div className="flex-1 flex flex-col gap-5 sm:gap-6">
                    ...
                    {!loading && !items.length && !error && <p className="text-center text-gray-500">Seu carrinho está vazio.</p>}
                    {items.map((it) => (
            ...
            <p className="text-[10px] text-gray-400 text-center">
                (Implementação simplificada: quantidade só local, sem associação real a usuário. Facilita refatorar depois.)
            </p>
        </div>
    );
}