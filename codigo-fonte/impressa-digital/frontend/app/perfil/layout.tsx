...
export default function LayoutPerfil({ children }: { children: ReactNode }) {
  ...
  // Estados para controlar dropdown e modais
  ...
  // Efeito para controlar modal de login
  useEffect(() => {
    if (!loading && !user) {
    ...
    <ProtectedRoute>
      <div className="min-h-screen bg-white flex flex-col">
        ...
        <div className="flex flex-1">
          <SidebarPerfil />
          <main className="flex-1 max-w-7xl p-6 font-sans mt-8 overflow-y-auto">{children}</main>
        </div>
        ...
        {showLoginModal && (
          ...
          >
            <div className="bg-white rounded-lg shadow-2xl p-6 w-full max-w-sm" onClick={(e) => e.stopPropagation()}>
              ...
              <p className="text-gray-600 mb-6">Para acessar a área de perfil, você precisa estar logado.</p>
              <div className="flex justify-end gap-3">
                <button
                  className="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition"
                  onClick={handleLoginModalClose}
