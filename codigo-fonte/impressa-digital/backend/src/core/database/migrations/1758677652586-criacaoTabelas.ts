...
export class CriacaoTabelas1758677652586 implements MigrationInterface {
    name = 'CriacaoTabelas1758677652586'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`produtos\` (\`id\` int NOT NULL AUTO_INCREMENT, \`categoria_id\` int NOT NULL, \`nome\` varchar(255) NOT NULL, \`descricao\` text NULL, \`preco\` decimal(10,2) NOT NULL DEFAULT '0.00', \`status_produto\` tinyint NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`user\` (\`id\` int NOT NULL AUTO_INCREMENT, \`name\` varchar(255) NOT NULL, \`email\` varchar(255) NOT NULL, \`phone\` varchar(255) NOT NULL, \`birthDate\` date NULL, \`cpf\` varchar(255) NULL, \`password\` varchar(255) NULL, \`role\` enum ('admin', 'cliente') NOT NULL DEFAULT 'cliente', \`criado_em\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), UNIQUE INDEX \`IDX_e12875dfb3b1d92d7d7c5377e2\` (\`email\`), UNIQUE INDEX \`IDX_a6235b5ef0939d8deaad755fc8\` (\`cpf\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
